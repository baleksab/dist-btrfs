...
export const BtrfsApiAxiosParamCreator = function (configuration?: Configuration) {
    return {
         ...
         * @throws {RequiredError}
         */
        apiBtrfsSubvolumesSubvolumeStorageMetricsGet: async (subvolume: string, serverUid?: string, options: RawAxiosRequestConfig = {}): Promise<RequestArgs> => {
            // verify required parameter 'subvolume' is not null or undefined
            assertParamExists('apiBtrfsSubvolumesSubvolumeStorageMetricsGet', 'subvolume', subvolume)
            const localVarPath = `/api/btrfs/subvolumes/{subvolume}/storageMetrics`
                .replace(`{${"subvolume"}}`, encodeURIComponent(String(subvolume)));
            // use dummy base URL string because the URL constructor only accepts absolute URLs.
            const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL);
            let baseOptions;
...
export class BtrfsApi extends BaseAPI {
     ...
     * @throws {RequiredError}
     */
    public apiBtrfsSubvolumesSubvolumeStorageMetricsGet(subvolume: string, serverUid?: string, options?: RawAxiosRequestConfig) {
        return BtrfsApiFp(this.configuration).apiBtrfsSubvolumesSubvolumeStorageMetricsGet(subvolume, serverUid, options).then((request) => request(this.axios, this.basePath));
    }
}
